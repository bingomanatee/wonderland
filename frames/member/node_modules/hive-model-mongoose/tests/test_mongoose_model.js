				...
				Back_Model.add({name: 'alpha', tags: ['a', 'b'], notes: 'n'},
					function (err, alpha) {
						...
						Back_Model.archive(alpha, ['tags'], new_data, function (err, arch_alpha) {
							...
							delete j.__v;
							delete(j._id);
							t.deepEqual(j, {name: 'alpha', tags: ['c', 'd'], notes: 'n', _archives: [
								{tags: ['a', 'b']}
							]}, 'archived alpha');
							_try_drop();
							t.end();
		...
		Mongoose_Model(
			{
				...
				mongoose:   mongoose,
				schema_def: {name: { type: 'string', index: {unique: true}}, deleted: 'boolean'}
			}, dataspace, function (err, Foo_Model) {
...
if (true) {
	tap.test('add ', function (t) {
		...
		Mongoose_Model(
			...
			},
			{
			...
			}, dataspace, function (err, Bar_Model) {
				...
				Bar_Model.add([
					...
					function (err, results) {
						...
						t.equal(results.length, 3, 'three records added');
						...
						Bar_Model.add({name: 'Mickey Mouse', type: 'strip'}, function (err, mouse) {
							...
							Bar_Model.add([
								...
								function (err, results) {
									t.ok(err, 'Broke #1 rule of fight club (capacity is a string)');
									_try_drop();
									t.end();
...
if (true) {
	...
	tap.test('revise', function (t) {
		...
		Mongoose_Model(
			...
			, dataspace, function (err, Bing_Model) {
