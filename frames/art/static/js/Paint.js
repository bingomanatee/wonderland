...
(function (window) {
    ...
    Drag_Box.prototype = {update: function () {
    ...
    function make_boxes(manager) {
        manager.boxes = [new Drag_Box(manager, 0, 0), new Drag_Box(manager, 0, 1), new Drag_Box(manager, 1, 0), new Drag_Box(manager, 1, 1)]
    }
...
(function (window) {
    ...
    angular.module("Paint").factory("Paint_Manager_Polygon", function () {
        ...
        function _pp_click(manager) {
            return function (event) {
                ...
                if (manager.poly_point_mode == "add") {
                    ...
                    shape.graphics.f(WHITE).r(-POINT_SIZE / 2, -POINT_SIZE / 2, POINT_SIZE, POINT_SIZE).s(OUTLINE).ss(1).r(-POINT_SIZE / 2, -POINT_SIZE / 2, POINT_SIZE, POINT_SIZE);
        ...
        return function (manager) {
            ...
            var pp_shape = new createjs.Shape;
...
(function (window) {
    ...
    angular.module("Paint").factory("Paint_Manager_Leap", function () {
        ...
        function _z_bubble(manager) {
            ...
            manager.z_bubble_axis.graphics.s(LEAP_GUIDE_COLOR2).ss(2).mt(manager.screen_width() / -2, 0).lt(manager.screen_width() * -Z_INNER_PERCENT, 0).es().s(LEAP_GUIDE_COLOR).mt(manager.screen_width() * Z_INNER_PERCENT, 0).lt(manager.screen_width() / 2, 0).es();
        ...
        function _smooth(old_value, new_value, manager) {
            if (!isNaN(new_value)) {
                if (isNaN(old_value)) {
                    ...
                    return manager.snap(.8 * old_value + .2 * new_value)
                }
            } else {
        ...
        function _set_guides(manager, point, which) {
            ...
            this[inertia_key] = Math.max(1, Math.min(1e3, this[inertia_key]));
...
(function (window) {
    ...
    Point_Manager_Shape.prototype = {clone: function (not_root) {
        var clone;
        switch (this.type) {
            case"group":
        ...
        switch (this.type) {
            ...
            case"triangle":
                this.shape.graphics.c().f(this.get_color());
                this.shape.graphics.mt(-x2, y2).lt(0, -y2).lt(x2, y2);
                this.shape.graphics.ef();
                break;
            case"polygon":
                this.shape.graphics.c().f(this.get_color());
...
(function () {
    ...
    Color_Palette.prototype = {ld: function (hue) {
        var self = this;
        return function (event) {
...
(function (window) {
    ...
    angular.module("Paint").factory("Paint_Manager", function (Paint_Manager_Grid, Paint_Manager_Shape, Paint_Manager_Polygon, Paint_Manager_Boxes, Paint_Manager_Leap, Color_Palette, Paint_Manager_Move) {
        function Paint_Manager(params) {
            ...
            this.palette = Color_Palette(this, this.margin() * 2, this.margin() * 2, this.screen_width() - 4 * this.margin(), this.screen_height() - 4 * this.margin());
            Paint_Manager_Leap(this);
            this.update()
        }

        Paint_Manager.prototype = {"export": function () {
...
(function () {
    ...
    paint.directive("paintEditor", function InjectingFunction(Paint_Manager) {
        return{templateUrl: "/js/art/directives/editor.html", compile: function CompilingFunction($templateElement, $templateAttributes) {
            return function LinkingFunction($scope, $linkElement, $linkAttributes) {
                $scope.show_drawing = true;
                $scope.tab_class = function (which) {

                    var classes = ['tab'];
                    if (which == $scope.tab) classes.push('active');
                    return classes.join(' ');
                };
